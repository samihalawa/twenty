...
await test('paid OpenRouter GPT-OSS sets supported low reasoning effort',async()=>{
 ...
 assert.equal(JSON.stringify(modelConfig.getReasoningProviderOptions(m)),JSON.stringify({openrouter:{reasoning:{effort:'low',exclude:false},provider:{only:['CoreWeave','DeepInfra'],order:['CoreWeave','DeepInfra'],require_parameters:true,max_price:{prompt:0.05,completion:0.20}}}}));
...
await test('installed compatible SDK forwards the paid provider allowlist into the actual request',async()=>{
 ...
 assert.equal(req.args.response_format.type,'json_schema');assert.equal(req.args.reasoning_effort,undefined);assert.equal(JSON.stringify(req.args.reasoning),JSON.stringify({effort:'low',exclude:false}));
