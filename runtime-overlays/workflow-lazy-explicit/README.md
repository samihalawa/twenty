...
The case-context runtime capability rejects any mode except `READ_CASE` and all unlisted arguments before dispatch. Gmail draft proof includes `threadContextFingerprint` from a complete native thread read, excluding the exact current draft and ignoring ephemeral labels. Exact draft sending requires `approvedThreadContextFingerprint`; new messages or changes to another draft block dispatch immediately before the sole send attempt.
